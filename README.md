...
- Exactly-once deferred completion follow-ups, a running-count widget, and a read-only `/ps` picker/detail view.
...
Use `/ps` to inspect live stdout/stderr or kill a terminal interactively. The model can use `bg_status`, `bg_list`, and `bg_kill` directly. Prefer Pi's regular shell tool for quick commands.
